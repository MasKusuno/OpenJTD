...
- [x] `shanai_lan` の source-backed FDMVector drawing primitives を promote する。model は two-point `0xff000160` line records、top-level `0xff000660`/`0xff000960` path records、`0xff000460`/`00000460` ellipse records、`0xff000a60` compound records 内の nested `00000660`/`00000960` と nested `ff000660`/`ff000960` primitives を保存する。`0xff000960`/`00000960` tails は cubic control-handle deltas として保持されるため、globe/orbit curves は angular polyline ではなく cubic Bezier として render される。`getPageLayerTree` は 1889 件の `fdmVectorPrimitiveProjection` ops を `geometryDecoded:true`、`primitiveKind`、`curveSegmentCount`、`styleWord`、BGR interpreted `fillColor`/`strokeColor`、`strokeWidth`、`pathClosed`、source ellipse geometry、reference-backed `projectionViewport` evidence 付きで expose する。SVG/PDF は old blue bbox/path debug overlays ではなく source-backed strokes、conservative closed-path fills、ellipse dots/circles を render し、latest `shanai_lan` 144dpi raster diff mean は約 `24.35`、right globe crop RMS は cubic-handle projection 後 `103.85` から `96.07` へ改善した。
- [x] source-backed `shanai_lan` `DocumentText` labels を同じ reference-backed diagram viewport に合わせ、proven text fragments (`社内LAN構成図` green、`ファイルサーバ`/`ｶﾗｰｼﾞｪｯﾄﾌﾟﾘﾝﾀｰ` blue) の conservative label colors を `textRun.fillColor` と SVG/PDF text output に保存する。generated title の 144dpi green bbox はおおよそ `(109,111)` から `(70,62)` へ移動し、reference `(69,58)` に近づいた。
...
- [ ] 残る `shanai_lan` connector graph semantics を decode する。recovered FDMVector primitives は device silhouettes、一部の fills、BGR color families、width states を再構成するが、grouped connector ownership、text object styling/placement、complete label semantics、exact stroke style semantics、image/object layering、paint order はまだ不足している。
- [ ] `shanai_lan` label placement は `/DocumentText` を plain fixed-width text grid として扱わず、real anchors から decode する。新しい Microsoft Print To PDF reference は PDF text layer に full LAN labels が入っていることを確認したが、全 `DocumentText` lines を fixed grid として trial projection すると 144dpi raster mean diff はわずかに悪化 (`24.13` -> `24.40`) し、labels も visibly misplaced になった。remaining labels を render する前に `/DocumentTextPositionTables`、FDM text-object records、または FDM row/object anchors を使う。
