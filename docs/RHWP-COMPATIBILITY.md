...
- Parser/export/app-core JSON also preserves decoded-false `/Frame` fixed 60-byte records as `objectFrameRecords`. `rjtd object-fdm-frame-links <file>` correlates image-bearing FDMIndex rows with those records by `fdm row index == frame object id`. The 61-sample sweep has 0 failures and links all 6 FDM image rows to frame records, with 0 missing-frame rows, 11 complete payloads, and 0 renderable rows. This proves a stronger placement evidence trail, but geometry units, page association, paint order, and payload-to-image selection still must be decoded before rendering these images.
