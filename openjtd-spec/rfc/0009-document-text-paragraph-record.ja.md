...
**表行ヘッダーのサブ構造（decoded:false）。** `03新旧（整備令）.jtd` において `0x001c/0x0030` セルヘッダー列の直前に現れるレコードは、固有のヘッダー形状を共有する：`w4=0x008f`、`w5=(len-12)`、`w6=0x010c(268)`。式 `len = w5 + 12` はこのサンプルで確認された 154 件の表行レコードすべてで成立する。フィールド `w6=268` は後続するセルレコード全体の `b1` 最大値に等しく、`w6` が表の全幅を右端座標としてエンコードしている仮説と整合する。`w8` はペイロード内の `0x0023` タグ付きサブエントリ数に等しい。サブエントリには `0x0023`（値 0/2/6 の疎なエントリ）と `0x002b`（コンテンツ列幅に近い値）の 2 種のタグが観測された。ペイロードは `0xffff` センチネルで終わる。サブエントリフィールドの詳細な意味は未解読。
