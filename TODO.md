...
- [ ] Decode the remaining `shanai_lan` connector graph semantics: the recovered FDMVector primitives now reconstruct most device silhouettes, some fills, BGR color families, and width states, but grouped connector ownership, text object styling/placement, complete label semantics, exact stroke style semantics, image/object layering, and paint order are still missing.
- [ ] Decode `shanai_lan` label placement from real anchors instead of treating `/DocumentText` as a plain fixed-width text grid. The new Microsoft Print To PDF reference confirms that the PDF text layer contains the full LAN labels, but a trial projection of all `DocumentText` lines as a fixed grid made the 144dpi raster mean diff slightly worse (`24.13` -> `24.40`) and visibly misplaced labels. Next work should use `/DocumentTextPositionTables`, FDM text-object records, or FDM row/object anchors before rendering the remaining labels.
