...
- [x] Sweep JSON export for model-preserved `imagePayloads`: 61 checked, 0 failures, 14 payload files, 94 complete payloads, 373,466 bytes, JPEG 62 rows/8 files, GIF89a 31 rows/9 files, GIF87a 1 row/1 file.
...
- [x] Sweep image payload envelopes: 61 checked, 0 failures, 94 envelopes, 6 `le32` declared length matches, all currently in `Embedding */Contents` JPEG rows.
...
- [x] Sweep ownership candidates: 61 checked, 0 failures, 933 candidates, 474 with ownership, and all 94 image payload rows covered by an ownership candidate (`contents` 67, `embedded-press` 8, `fdm-vector` 19).
...
- [x] Sweep ownership references: 61 checked, 0 failures, 12 files with references, 56 embedded image candidates with references, 646 reference rows, 10,055 total byte matches, and 75 of 94 image payload rows covered by cross-stream reference candidates.
...
- [x] Sweep `object-ownership-references`: 61 checked, 0 failures, 12 files with rows, 3,273 reported preview offsets; target families include `figure-data` 1,021, `frame` 941, `layout-box` 538, `page-mark` 502, `paper-mark` 158, and `figure` 113.
...
- [x] Record the strongest cross-row-free stride candidates with stride >= 12: `frame/u16-le/12/5` has 106 weighted matches across 12 files, `frame/u16-be/12/7` has 95 across 9 files, and `frame/u16-be/20/15` has 74 across 9 files.
...
- [x] Sweep `object-frame-reference-records`: 61 checked, 0 failures, 12 files with candidate records, 275 expanded rows. Candidate counts are `u16-le/12/5` 106, `u16-be/12/7` 95, and `u16-be/20/15` 74.
...
- [x] Sweep JSON export for model-preserved `frameReferenceRows`: 61 checked, 0 failures, positive files 12, rows 275, suffix links 69. Family counts match the CLI sweep exactly.
- [x] Add `rjtd object-image-frame-candidates <file>` to summarize each image payload source against model-owned `/Frame` row evidence, payload kinds, row families, row20 suffix links, and coordinate-looking row12 pairs.
- [x] Sweep `object-image-frame-candidates`: 61 checked, 0 failures, 14 files with image payload sources, 60 image sources, 56 frame-linked sources, 4 missing-frame sources, 275 frame rows. Preferred diagnostic buckets are `row12-tail-coordinate` 27, `row12-tail-zero` 8, `u16-le-row12` 20, and `none` 5.
- [x] Record that `row12-tail-coordinate` is a strong placement-analysis candidate but not yet sufficient for rendering: `FDMVector` sources account for most `none` rows, and `u16-le-row12` rows are mostly index/flag-like families without coordinate-looking pairs.
